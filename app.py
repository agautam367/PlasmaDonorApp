...
def check(email):
    url = "https://2zttulrz50.execute-api.us-east-2.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://2zttulrz50.execute-api.us-east-2.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://2zttulrz50.execute-api.us-east-2.amazonaws.com/plasma/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://2zttulrz50.execute-api.us-east-2.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    ...
    phone=[]
    for i in a:
        url="https://www.fast2sms.com/dev/bulk?authorization=ONfP1Lq9Jkx2C7jduGmit35BcYv46bzKhZeVrnQ0oXlMHAaT8WJYDRpmQz41orVsEKgtcObd0CyGa3XN&sender_id=FSTSMS&message=Plasma of your blood group has been requested&language=english&route=p&numbers="+str(i['phone'])
